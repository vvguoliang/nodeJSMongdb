...
// 上传文件
routerAdmin.get('/fileManagement', function (req, res, next) {
    // 从数据库中读取所有分类数据
    var reqPage = Number((req.query.page) === undefined ? 0 : req.query.page);
    var page = reqPage <= 0 ? 1 : reqPage;
    var limit = 20;
    ...
    var skip = (page - 1) * limit;
    //
    FileMangement.count().then(function (count) {
        // console.log(count);
        //总页数
        pages = Math.ceil(count / limit);
        User.find().sort({ _id: -1 }).then(function (users) {
            FileMangement.find().sort({ _id: -1 }).limit(limit).skip(skip).then(function (fileManagements) {
                // console.log('分类首页回显数据  ' + categories);
                if (fileManagements.username === users.username) {
                    res.render('admin/fileManagement_index', {
                        userInfo: req.userInfo,
